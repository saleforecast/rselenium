...
RUN R -e "install.packages(c('shiny', 'RSelenium', 'wdman', 'httr', 'netstat'), dependencies=TRUE)"
...
CMD ["java", "-jar", "/usr/bin/selenium-server-standalone.jar", "-port", "4567", "&", "R", "-e", "shiny::runApp('app.R', port = 5000, host = '0.0.0.0')"]
